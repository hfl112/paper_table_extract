...
| ~~**更多横向转正表**~~ **已补齐（2026-07-30）** | F-001 的分母原本只有开发集 3 张 + 留出集 1 张。PPTP 语料（`gen/`）一次给出 **22 个转正页 / 16 篇**；深挖的 4 张里 **3 张复现 F-001**。顺带把 `VERTICAL_LINES_THRESHOLD` 的下界从 82 拉到 **40**（余量 2.7x → **1.33x**）。见 `log.md` §41.6 |
